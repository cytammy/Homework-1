...
histogram <- ggplot(mpg, aes(x=hwy)) + geom_histogram(binwidth = 1, color = 'white', fill = 'dodgerblue3')
histogram
```
...
box <- ggplot(data = mpg, aes(x = cyl, y = hwy)) + geom_boxplot(aes(group = cyl), fill = 'tan3')
box
```
...
```{r}
var <- c('hwy', 'cyl', 'displ', 'cty') # selecting variables without categorical values
M <- cor(mpg[var])
corrplot(M, method = 'square', order = 'FPC', type = 'lower', diag = FALSE) # creating the diagonal plot
```
